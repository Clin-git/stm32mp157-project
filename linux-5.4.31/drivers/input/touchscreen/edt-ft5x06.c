...
struct edt_ft5x06_ts_data {
	...
	bool exchange_xy;		/* 自己添加的变量，用于判断是否交换读取到的x值和y值 */
};
...
static irqreturn_t edt_ft5x06_ts_isr(int irq, void *dev_id)
{
	...
	for (i = 0; i < tsdata->max_support_points; i++) {
		...
		/* tsdata->exchange_xy是自己添加的变量，因为本人用的lcd是反过来的 */
		if (tsdata->exchange_xy)
		{
					 ...
					 const struct i2c_device_id *id)
{
	...
	tsdata->exchange_xy = (of_device_is_compatible(client->dev.of_node, "edt,edt-ft5426") > 0);	/* 自己添加的代码 */
...
static const struct of_device_id edt_ft5x06_of_match[] = {
	...
	{ .compatible = "edt,edt-ft5426", .data = &edt_ft5x06_data },
	{ /* sentinel */ }
};
